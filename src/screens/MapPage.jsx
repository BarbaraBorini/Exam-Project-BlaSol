/**
 * MapPage.jsx – The main map screen
 *
 * Renders the festival map with:
 *  - Stage, bar, toilet, merch, food and water items placed at fixed coordinates
 *  - A filter bar (Scener / Toiletter / Barer) that shows/hides categories
 *  - A group banner or group bar at the top (depending on whether the user has a group)
 *  - Friend markers on the map when a group is active
 *  - A meetup-point flow: "placing" mode → click to choose spot → modal to set time
 */
...
// Map icons (local SVG files from /src/assets/)
...
// The main map SVG – includes background, paths, North sign,
// "You" marker, AID icon and entrance (INDGANG) signs
...
// Stage background banners (one per stage)
...
// Shared UI icons
import { ICON_FILTER, ICON_BANNER_GROUP, ICON_CHEVRON_DOWN, BACK_ARROW } from '../assets'

// ── Group Banner ──────────────────────────────────────────────────────────────
//
// Shown at the top of the map when the user has NOT yet joined or created a group.
// It's an accordion: the info text expands/collapses when the chevron is tapped.
...
// ── My Group Bar ──────────────────────────────────────────────────────────────
//
// Shown at the top of the map AFTER the user has joined/created a group.
// Displays the group name, member list, and action buttons.
...
// ── Friend Markers ────────────────────────────────────────────────────────────
//
// Circular markers placed on the map for each group member except the current user.
// The current user's position is already shown by the "You" marker inside Map.svg.
// Tapping a marker toggles the member's name label.
...
function FriendMarkers({ members, visible, selectedFriendId, onFriendClick }) {
  ...
  // Only show markers for friends (not the current user)
...
// ── Queue Tag ─────────────────────────────────────────────────────────────────
//
// A small coloured badge shown above bars and toilets when their filter is active.
// Displays 3 person icons; the number that are filled (dark blue) shows queue length.
//   short  → 1 filled  (green background)
//   medium → 2 filled  (orange background)
//   long   → 3 filled  (red background)

// SVG paths taken directly from assets/person.svg (viewBox 0 0 7 12)
...
// ── Artist Tag ────────────────────────────────────────────────────────────────
//
// A small pink label shown above stage items when the Scener filter is active.
// "inset" controls how many pixels the tag overlaps the top edge of the stage
// (used to fine-tune positioning for rotated stages like BIRKELUNDEN).
...
// ── Filter Bar ────────────────────────────────────────────────────────────────
//
// A row of pill buttons that let the user filter map items by category.
// When any filter is active, the left button becomes "× Ryd" to clear all filters.
// Only one category is shown at a time: the selected one, everything else hides.
...
// ── Map Item ──────────────────────────────────────────────────────────────────
//
// A generic icon + label placed at an absolute position on the map.
// Used for bars, toilets, merch, food, and water.
//
// hidden      → renders nothing (used when a different filter is active)
// highlighted → adds a drop shadow (used when this item's category is filtered)
// queue       → shows a QueueTag above the item ('short' | 'medium' | 'long')
...
// ── Stage Item ────────────────────────────────────────────────────────────────
//
// A stage banner placed at an absolute position on the map.
// Each stage has a custom background SVG, optional rotation, and an ArtistTag
// that appears above it when the Scener filter is active.
//
// tagInset → controls how many pixels the ArtistTag overlaps the stage edge.
//            Higher values push the tag further into the stage (used for BIRKELUNDEN
//            which is rotated and needs extra offset to look correctly placed).
...
// ── Meetup flow sub-components ────────────────────────────────────────────────
//
// The meetup flow has three states managed by `meetupMode`:
//   null      – no meetup placement in progress
//   'placing' – a transparent overlay covers the map; clicking places a pin
//   'modal'   – the time picker modal is open for the user to set the meetup time
//
// Once confirmed, meetupPoint is saved to localStorage and shown on the map permanently.

const MEETUP_HINT_TEXT = 'Click where you want to create your meet up point.'
...
// Hours 00-23 and minutes in 5-minute steps (00, 05, 10 … 55)
...
function MeetupTimeField({ time, onTimeChange }) {
  ...
  const handleHourChange = e => {
    ...
    if (!nextHour) { onTimeChange(''); return }
    onTimeChange(`${nextHour}:${minute || '00'}`)
  }

  const handleMinuteChange = e => {
    const nextMinute = e.target.value
    if (!nextMinute) { onTimeChange(hour ? `${hour}:00` : ''); return }
  ...
  return (
    <div className="meetup-modal__time-field">
      ...
      <div className="meetup-modal__time-selects">
        <select className="meetup-modal__time-select" value={hour} onChange={handleHourChange} aria-label="Hour">
          <option value="" disabled>HH</option>
          {MEETUP_HOURS.map(h => <option key={h} value={h}>{h}</option>)}
        </select>
        <span className="meetup-modal__time-separator">:</span>
        <select className="meetup-modal__time-select" value={minute} onChange={handleMinuteChange} aria-label="Minute">
          <option value="" disabled>MM</option>
          {MEETUP_MINUTES.map(m => <option key={m} value={m}>{m}</option>)}
        </select>
      </div>
    </div>
  )
...
// The pin marker shown on the map.
// `preview` is used for the "ghost" pin shown while the modal is open.
// When not in preview mode it's a button that toggles the time label.
...
// Default state: all three filters are off (nothing is filtered)
...
export default function MapPage({
  ...
  // Filter state – at most one filter is active at a time
  ...
  // Friend visibility toggle (the eye button in the group bar)
  const [friendsVisible, setFriendsVisible] = useState(true)

  // Meetup flow state machine: null | 'placing' | 'modal'
  ...
  // Which friend marker is currently selected (shows name label)
  const [selectedFriendId, setSelectedFriendId] = useState(null)

  const mapContainerRef = useRef(null)

  // Keep a ref to meetupMode so the click handler can read the latest value
  // without needing to be re-created every render (avoids stale closure issues)
  const meetupModeRef = useRef(meetupMode)
  useEffect(() => { meetupModeRef.current = meetupMode }, [meetupMode])

  // Persist meetup point to localStorage whenever it changes
  useEffect(() => {
    saveMeetupPoint(meetupPoint)
  }, [meetupPoint])

  // Toggle meetup placement mode on/off
  const handleNewMeetup = () => {
    if (meetupMode === 'placing') {
      // Already in placing mode – cancel it
      setMeetupMode(null)
  ...
  // Called when the user clicks on the map while in 'placing' mode.
  // Records the clicked position and opens the time picker modal.
  ...
  // "Cancel" in the modal → go back to placing mode
  ...
  // "Create" in the modal → save the meetup point and exit the flow
  ...
  // Tap an existing meetup marker to toggle its time label
  const handleMeetupMarkerClick = e => {
    e.stopPropagation()
    setMeetupTimeVisible(prev => !prev)
  }

  // Tap a friend marker to toggle their name label
  ...
  // ── Filter helpers ──────────────────────────────────────────────────────────
  //
  // When any filter is active, we hide items that don't match the active category.
  // Items with no filter category (merch, food, water) are always hidden when
  // any filter is active to keep the map clean.
  //
  // Usage pattern:
  //   hidden={anyFilterActive && !activeFilters.bars}   ← hide when bars filter is NOT active
  //   hidden={anyFilterActive}                           ← always hide when any filter active
  ...
  return (
    <div className="screen map-screen">
      ...
      {/* ── Map container ── */}
      {/* Items inside here are placed at absolute pixel coordinates.
          The coordinates were measured against the Map.svg at its rendered size.
          Do NOT change top/bottom on .map-container without adjusting both equally,
          or the items will drift out of alignment with the underlying map. */}
      <div ref={mapContainerRef} className="map-container">
        ...
        {/* ── Other items – hidden whenever any filter is active ── */}
        ...
        {/* ── Friend markers (only when the user has a group) ── */}
        ...
        {/* Ghost pin shown while the meetup modal is open */}
        ...
        {/* Saved meetup point pin – tapping it shows/hides the time label */}
        {meetupPoint && (
          <MeetupMarker
        ...
        {/* Transparent click-capture overlay active during 'placing' mode */}
        {meetupMode === 'placing' && (
          <div
      ...
      {/* Time picker modal – rendered outside map-container so it's not clipped */}
      {meetupMode === 'modal' && (
        <MeetupModal
          time={meetupTime}
      ...
      {/* ── Overlays (group bar/banner + filter bar) ──
          These sit above the map at z-index 5, so they are always visible
          regardless of what is happening on the map below. */}
      <div className="map-overlays">
        ...
        <div className="map-filter-bar">
          <FilterBar
            ...
            onToggle={id => setActiveFilters(prev => ({ ...prev, [id]: !prev[id] }))}
            onReset={() => setActiveFilters(ALL_FILTERS_OFF)}
          />
        </div>

        {/* Hint text shown while the user is in meetup placing mode */}
